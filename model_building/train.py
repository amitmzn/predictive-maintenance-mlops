...
if __name__ == "__main__":
    ...
    df = pd.read_csv("hf://datasets/amitmzn/Predictive-Maintenance/engine_data.csv")
    ...
    # Threshold tuned for 90% recall (high sensitivity — minimises missed faults)
    ...
    print(f"Validation AUC: {roc_auc_score(y_val, xgb_val_prob):.4f}")
    print(f"Optimal Threshold (90% Recall): {optimal_threshold:.4f}")
    ...
    }, os.path.join(model_dir, "preprocessing_bounds.joblib"))

    print("6. Uploading to Hugging Face Hub...")
    api = HfApi(token=os.getenv("HF_TOKEN"))
    for filename in ["best_maintenance_model.joblib", "scaler.joblib", "imputer.joblib", "preprocessing_bounds.joblib"]:
        api.upload_file(
    ...
    print(f"Done. All artifacts uploaded to {MODEL_REPO}")
